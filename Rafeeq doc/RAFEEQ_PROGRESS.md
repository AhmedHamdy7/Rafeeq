...
**الوضع:** ✅ **Phase 0 · 1 · 2 · 3 · 4 مكتملات + تسليم OpenAPI.** فاضل **11 مرحلة** من 16 (0–15). للـ MVP القابل للإطلاق: المراحل 5–9 + الأجزاء الحرجة من 11 و13.
73 migration · 71 جدول دومين · 71 موديل · 82 enum · 71 factory · **27 endpoint** · **600 اختبار كلهم خضرا**.
...
`users.trust_level` = عدد المعتمَد (0–4)، بيتحسب من الصفوف نفسها كل مرة مش بـ increment — عدّاد بيتزوّد مرتين أو بيفوت مرة بيسيب رقم غلط للأبد. **مختلف تمامًا** عن `trust_scores.score` (0–100 داخلي، Phase 10).
...
| # | القرار | السبب |
|---|---|---|
| 41 | **ترتيب الرفع: افحص → جرّد الميتاداتا → خزّن** | الفحص بعد إعادة الترميز بيفحص ملف إحنا عملناه — مابيثبتش حاجة عن اللي اترفع. والتخزين قبل الفحص بيحط بايتات مفحوصة في الـ bucket، و"مسحناها بعدين" مش زي "عمرها ما اتخزنت" |
| 42 | **إعادة ترميز الصورة عبر GD** | مش تجميل: صورة البطاقة من موبايل بتحمل إحداثيات GPS في الـ EXIF — غالبًا بيت الشخص، لأن ده المكان اللي بيصوّروا فيه مستنداتهم. تخزينها جنب الرقم القومي بيدّي للمهاجم أكتر من المستند نفسه، والشخص عمره ما وافق على ده. فك الترميز لـ bitmap وإعادة الترميز بتسيب البيكسلات بس |
| 43 | **مسار التخزين مالوش معنى** | اسم عشوائي تحت id المالك — مش نوع المستند ولا اسم الشخص ولا اسم الملف الأصلي. سرد الـ bucket مابيكشفش حاجة، والمسار المسرّب مايتعدّلش لتخمين الجار |
...
### 🎯 الخطوة الجاية
**Phase 5 — الأماكن والـ Corridors ونشر الرحلات (الفصل 4).** الفصل 3 بيقفل بشرط: "Vehicles become available for creating Commutes in Chapter 4" — متحقق (عربية معتمَدة + نشطة).
